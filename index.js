...
app.get('/api/status', authApi, (_req, res) => {
  res.json({
    ok: true,
    connected,
    status: connected ? 'online' : 'connecting',
    ...
    pairingAvailable: Boolean(sock && !connected)
  })
})

app.get('/api/qr', authApi, (_req, res) => {
  if (!latestQr) return res.status(404).json({ ok: false, error: 'QR belum tersedia' })
  res.json({ ok: true, qr: latestQr })
})

app.post('/api/pairing', authApi, async (req, res) => {
  try {
    if (!sock) return res.status(503).json({ ok: false, error: 'WhatsApp engine belum siap' })
    if (connected) return res.status(409).json({ ok: false, error: 'Bot sudah terhubung' })
    let number = String(req.body?.phone || '').replace(/\D/g, '')
    if (number.startsWith('0')) number = '62' + number.slice(1)
    if (number.startsWith('8')) number = '62' + number
    if (number.length < 10) return res.status(400).json({ ok: false, error: 'Nomor WhatsApp tidak valid' })

    const code = await sock.requestPairingCode(number)
    latestPairingCode = code
    pairingRequested = true
    res.json({ ok: true, code, phone: number, expiresIn: 60 })
  } catch (error) {
    logger.error({ err: error }, 'API pairing request failed')
    res.status(500).json({ ok: false, error: error?.message || 'Gagal membuat pairing code' })
  }
})
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    if (qr) {
      ...
      if (!PAIRING_NUMBER) qrcode.generate(qr, { small: true })
    }
    if (connection === 'connecting') { connected = false; logger.info('WhatsApp: connecting...') }
    if (connection === 'open') { connected = true; pairingRequested = false; latestQr = null; latestPairingCode = null; startedAt = Date.now(); logger.info(`✅ ${BOT_NAME} ONLINE`) }
    if (connection === 'close') {
      connected = false
